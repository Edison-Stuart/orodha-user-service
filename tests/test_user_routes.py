from .fixtures import MOCK_DATA, TEST_USER_ID
from http import HTTPStatus

USER_ROUTE = "/api/v1/users"


def test_get(mock_service_env, mock_app_client, mock_create_keycloak_connection):
    """
    ...
    mock_app_client.post(
        USER_ROUTE, follow_redirects=True, json=MOCK_DATA["post_user_request"]
    )

    response_get = mock_app_client.get(
        f"{USER_ROUTE}/{TEST_USER_ID}", headers={"Content-Type": "application/json"}
    )
...
def test_delete(mock_service_env, mock_app_client, mock_create_keycloak_connection):
    """
    ...
    response_delete = mock_app_client.delete(
        f"{USER_ROUTE}/{user_data['id']}", headers={"Content-Type": "application/json"}
    )
    assert response_delete.text == ""
    assert response_delete.status_code == HTTPStatus.OK


def test_get_keycloak_error(mock_service_env, mock_app_client, mock_get_keycloak_error):
    """
    ...
    """
    response_get = mock_app_client.get(ROUTE_WITH_ID, follow_redirects=True)
    assert response_get.status_code == HTTPStatus.BAD_REQUEST


def test_get_mongo_error(mock_service_env, mock_app_client, mock_get_mongo_error):
    """
...
def test_post_keycloak_error(
    mock_service_env, mock_app_client, mock_post_keycloak_error
):
    """
...
def test_delete_does_not_exist(
    mock_service_env, mock_app_client, mock_delete_does_not_exist
):
    """
...
def test_delete_validation_error(
    mock_service_env, mock_app_client, mock_delete_validation_error
):
    """
